...
class OnPolicyDistillActor:
    ...
    def forward_backward_batch(
        ...
        teacher_id_to_name = data.meta_info.get("teacher_id_to_name", {})
        ...
        if use_dynamic_bsz:
            ...
            if mpu.is_pipeline_last_stage():
                teacher_ids_tensor = None
                if "teacher_name" in data.non_tensor_batch and "teacher_name_to_id" in data.meta_info:
                    teacher_name_to_id = data.meta_info["teacher_name_to_id"]
                    teacher_ids_tensor = torch.tensor(
                        [teacher_name_to_id[str(name)] for name in data.non_tensor_batch["teacher_name"].tolist()],
                        dtype=torch.long,
                    )
                if "distill_weight" in data.non_tensor_batch:
                    distill_weights_tensor = torch.tensor(
                        np.asarray(data.non_tensor_batch["distill_weight"], dtype=np.float32), dtype=torch.float32
                    )
                else:
                    distill_weights_tensor = torch.ones(len(data.batch["input_ids"]), dtype=torch.float32)
                use_multi_teacher = "multi_teacher_topk_logps" in data.non_tensor_batch
                if use_multi_teacher:
                    teacher_topk_logps_tensor = torch.tensor(data.non_tensor_batch["multi_teacher_topk_logps"])
                    teacher_topk_indices_tensor = torch.tensor(data.non_tensor_batch["multi_teacher_topk_indices"])
                    teacher_weights_tensor = torch.tensor(data.non_tensor_batch["multi_teacher_weights"])
                    teacher_topk_logps, teacher_topk_indices = [], []
                    for partition in indices:
                        teacher_topk_logps.append(teacher_topk_logps_tensor[partition].contiguous())
                        teacher_topk_indices.append(teacher_topk_indices_tensor[partition].contiguous())
                else:
                ...
                for i, mb in enumerate(micro_batches):
                    ...
                    mb["distill_weights"] = (
                        distill_weights_tensor[indices[i]]
                        .view(-1, 1)
                        .expand(-1, mb["attention_mask"].size(1))
                        .clone()
                        .pin_memory()
                    )
                    if teacher_ids_tensor is not None:
                        mb["teacher_ids"] = (
                            teacher_ids_tensor[indices[i]]
                            .view(-1, 1)
                            .expand(-1, mb["attention_mask"].size(1))
                            .clone()
                            .pin_memory()
                    ...
                    if use_multi_teacher:
                        mb["multi_teacher_topk_logps"] = teacher_topk_logps[i].pin_memory()
                        mb["multi_teacher_topk_indices"] = teacher_topk_indices[i].pin_memory()
                        mb["multi_teacher_weights"] = teacher_weights_tensor[indices[i]].pin_memory()
                    else:
            ...
            if mpu.is_pipeline_last_stage():
                if "teacher_name" in data.non_tensor_batch and "teacher_name_to_id" in data.meta_info:
                    teacher_name_to_id = data.meta_info["teacher_name_to_id"]
                    teacher_ids = np.array(
                        [teacher_name_to_id[str(name)] for name in data.non_tensor_batch["teacher_name"].tolist()],
                        dtype=np.int64,
                    )
                    teacher_ids = np.array_split(teacher_ids, len(micro_batches))
                else:
                    teacher_ids = None
                if "distill_weight" in data.non_tensor_batch:
                    distill_weights = np.array_split(
                        np.asarray(data.non_tensor_batch["distill_weight"], dtype=np.float32), len(micro_batches)
                    )
                else:
                    distill_weights = np.array_split(
                        np.ones(data.batch["input_ids"].size(0), dtype=np.float32), len(micro_batches)
                    )
                use_multi_teacher = "multi_teacher_topk_logps" in data.non_tensor_batch
                if use_multi_teacher:
                    teacher_topk_logps = np.array_split(
                        data.non_tensor_batch["multi_teacher_topk_logps"], len(micro_batches), axis=0
                    )
                    teacher_topk_indices = np.array_split(
                        data.non_tensor_batch["multi_teacher_topk_indices"], len(micro_batches), axis=0
                    )
                    teacher_weights = np.array_split(
                        data.non_tensor_batch["multi_teacher_weights"], len(micro_batches), axis=0
                    )
                else:
                ...
                for i, mb in enumerate(micro_batches):
                    ...
                    mb["kl_losses"] = torch.zeros_like(calc_kl_mask, dtype=torch.float32)
                    mb["distill_weights"] = (
                        torch.tensor(distill_weights[i], dtype=torch.float32)
                        .view(-1, 1)
                        .expand(-1, mb["attention_mask"].size(1))
                        .clone()
                        .pin_memory()
                    ...
                    if teacher_ids is not None:
                        mb["teacher_ids"] = (
                            torch.tensor(teacher_ids[i], dtype=torch.long)
                            .view(-1, 1)
                            .expand(-1, mb["attention_mask"].size(1))
                            .clone()
                            .pin_memory()
                    ...
                    if use_multi_teacher:
                        mb["multi_teacher_topk_logps"] = torch.tensor(teacher_topk_logps[i]).pin_memory()
                        mb["multi_teacher_topk_indices"] = torch.tensor(teacher_topk_indices[i]).pin_memory()
                        mb["multi_teacher_weights"] = torch.tensor(teacher_weights[i]).pin_memory()
                    else:
        ...
        def loss_func(output):
            ...
            if distill_weights is not None:
                masked_weights = distill_weights[calc_kl_mask].to(torch.float32)
                mean_kl_loss = (masked_kl_lossed * masked_weights).sum() / masked_weights.sum().clamp_min(1e-8)
                stats.update(
                    {
                        "actor/kl_loss": mean_kl_loss.detach().item(),
                        "actor/kl_loss_unweighted": masked_kl_lossed.mean().detach().item(),
                    }
                )
            else:
                mean_kl_loss = masked_kl_lossed.mean()
                stats.update({"actor/kl_loss": mean_kl_loss.detach().item()})
            if teacher_ids is not None and teacher_id_to_name:
                masked_teacher_ids = teacher_ids[calc_kl_mask]
                if distill_weights is not None:
                    masked_weights_for_teacher = distill_weights[calc_kl_mask].to(torch.float32)
                else:
                    masked_weights_for_teacher = torch.ones_like(masked_kl_lossed, dtype=torch.float32)
                for teacher_id, teacher_name in teacher_id_to_name.items():
                    # Keep metric keys consistent across workers even when a teacher
                    # is absent in the local micro-batch; the cross-worker concat path
                    # assumes every metrics dict shares the same key set.
                    stats.setdefault(f"actor/kl_loss_{teacher_name}", 0.0)
                    stats.setdefault(f"actor/kl_loss_{teacher_name}_unweighted", 0.0)
                    stats.setdefault(f"actor/kl_token_count_{teacher_name}", 0)
                    
                    teacher_mask = masked_teacher_ids == teacher_id
                    teacher_token_count = teacher_mask.sum().item()
                    if teacher_token_count == 0:
                        continue
                    teacher_kl = masked_kl_lossed[teacher_mask]
                    teacher_weights = masked_weights_for_teacher[teacher_mask]
                    weighted_teacher_kl = (teacher_kl * teacher_weights).sum() / teacher_weights.sum().clamp_min(1e-8)
                    stats[f"actor/kl_loss_{teacher_name}"] = weighted_teacher_kl.detach().item()
                    stats[f"actor/kl_loss_{teacher_name}_unweighted"] = teacher_kl.mean().detach().item()
                    stats[f"actor/kl_token_count_{teacher_name}"] = teacher_token_count
        ...
        def forward_step(batch_iter, model):
            ...
            def logits_processor(
                ...
                if logits.shape[:2] != calc_kl_mask.shape[:2]:
                    # Some Megatron forward paths return logits as [seq, batch, vocab],
                    # while packed logits_processor_args are [batch, seq, ...].
                    # Align before boolean indexing to avoid broadcasting to [seq, seq, vocab].
                    if logits.dim() == 3 and logits.shape[0] == calc_kl_mask.shape[1] and logits.shape[1] == calc_kl_mask.shape[0]:
                        logits = logits.transpose(0, 1)
                assert logits.shape[:2] == calc_kl_mask.shape[:2]

                if multi_teacher_topk_logps is not None:
                    n_teachers = multi_teacher_topk_logps.size(2)
                    weighted_kl_losses = torch.zeros_like(kl_losses, dtype=torch.float32)
                    for teacher_idx in range(n_teachers):
                        ...
                        assert logits.shape[:2] == curr_teacher_topk_indices.shape[:2]
                        assert logits.shape[:2] == curr_teacher_topk_logps.shape[:2]

                        curr_teacher_kl_losses = vocab_parallel_kl_divergence(
                            logits.clone(),
                            curr_teacher_topk_logps,
                            curr_teacher_topk_indices,
                        )
                        weighted_kl_losses += multi_teacher_weights[:, :, teacher_idx] * curr_teacher_kl_losses
                    kl_losses[calc_kl_mask] = weighted_kl_losses[calc_kl_mask]
                else:
                    assert logits.shape[:2] == teacher_topk_indices.shape[:2]
                    assert logits.shape[:2] == teacher_topk_logps.shape[:2]
                    computed_kl_losses = vocab_parallel_kl_divergence(
                        logits, teacher_topk_logps, teacher_topk_indices
                    )
                    kl_losses[calc_kl_mask] = computed_kl_losses[calc_kl_mask]
                return {
                    "kl_losses": kl_losses,
            ...
            if mpu.is_pipeline_last_stage():
                ...
                if "multi_teacher_topk_logps" in batch:
                    logits_processor_args = {
                        ...
                        "distill_weights": batch["distill_weights"].to(device, non_blocking=True),
                        "multi_teacher_topk_logps": batch["multi_teacher_topk_logps"].to(device, non_blocking=True),
                    ...
                    if "teacher_ids" in batch:
                        logits_processor_args["teacher_ids"] = batch["teacher_ids"].to(device, non_blocking=True)
                else:
