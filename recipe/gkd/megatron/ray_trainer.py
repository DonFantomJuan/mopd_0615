...
from recipe.gkd.megatron.dataset_teacher_router import build_unique_teachers, load_dataset_teacher_router
from recipe.gkd.megatron.teacher_utils import get_teacher_knowledge, get_teacher_knowledge_by_teacher_name
...
class OnPolicyDistillTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        self.use_dataset_teacher_router = self.config.data.get("dataset_teacher_router", None) is not None
        self.dataset_teacher_router = None
        self.teacher_clients_by_name = None
        self.teacher_workers_by_name = None
        self.teacher_name_to_id = {}
        self.teacher_id_to_name = {}
        if self.use_dataset_teacher_router:
            self.dataset_teacher_router = load_dataset_teacher_router(self.config.data.dataset_teacher_router)
            unique_teachers = build_unique_teachers(self.dataset_teacher_router)
            ordered_teacher_names = list(unique_teachers.keys())
            self.teacher_name_to_id = {name: idx for idx, name in enumerate(ordered_teacher_names)}
            self.teacher_id_to_name = {idx: name for name, idx in self.teacher_name_to_id.items()}
            self.teacher_clients_by_name = {}
            self.teacher_workers_by_name = {}
            for teacher_name, teacher_info in unique_teachers.items():
                self.teacher_clients_by_name[teacher_name] = TeacherClient(
                    teacher_info["server_ip"],
                    teacher_info["server_port"],
                    n_server_workers=teacher_info["n_server_workers"],
                )
                self.teacher_workers_by_name[teacher_name] = teacher_info["n_server_workers"]
            self.teacher_infos = None
        else:
            self.teacher_infos = self._build_teacher_infos(self.teacher_config)
    ...
    def _build_teacher_infos(self, teacher_config):
        teachers_cfg = OmegaConf.select(teacher_config, "teachers")
        root_request_mode = teacher_config.get("request_mode", "parallel")
        root_num_microbatches = teacher_config.get("num_microbatches", 1)
        root_max_tokens = teacher_config.get("max_tokens", 1)
        root_n_server_workers = teacher_config.get("n_server_workers", 1)
        root_temperature = teacher_config.get("temperature", 1)
        root_only_response = teacher_config.get("only_response", False)
        root_max_seq_len = teacher_config.get("max_seq_len", None)

        def build_info(idx, cfg):
            name = cfg.get("name", f"teacher_{idx}")
            weight = float(cfg.get("weight", 1.0))
            if weight < 0:
                raise ValueError(f"Teacher '{name}' weight must be non-negative, got {weight}")

            n_server_workers = int(cfg.get("n_server_workers", root_n_server_workers))
            client = TeacherClient(
                cfg["server_ip"],
                cfg["server_port"],
                num_microbatches=cfg.get("num_microbatches", root_num_microbatches),
                max_tokens=cfg.get("max_tokens", root_max_tokens),
                n_server_workers=n_server_workers,
                temperature=cfg.get("temperature", root_temperature),
                only_response=cfg.get("only_response", root_only_response),
                max_seq_len=cfg.get("max_seq_len", root_max_seq_len),
            )
            return {
                "name": name,
                "weight": weight,
                "n_server_workers": n_server_workers,
                "request_mode": cfg.get("request_mode", root_request_mode),
                "client": client,
            }

        if teachers_cfg:
            teacher_infos = [build_info(idx, cfg) for idx, cfg in enumerate(teachers_cfg)]
        else:
            teacher_infos = [
                build_info(
                    0,
        ...
        total_weight = sum(info["weight"] for info in teacher_infos)
        if total_weight <= 0:
    ...
    def _async_get_teacher_knowledge(self, future: GenerationBatchFuture):
        ...
        if self.use_dataset_teacher_router:
            teacher_request_batch = gen_batch_output
            teacher_request_batch.non_tensor_batch["teacher_name"] = batch.non_tensor_batch["teacher_name"]
            teacher_request_batch.non_tensor_batch["distill_weight"] = batch.non_tensor_batch["distill_weight"]
            
            future.set_teacher_batch_output(
                get_teacher_knowledge_by_teacher_name(
                    teacher_request_batch,
                    self.teacher_clients_by_name,
                    self.teacher_workers_by_name,
                    is_async=True,
                )
            ...
            future.set_teacher_batch_output(
                get_teacher_knowledge(gen_batch_output, self.teacher_infos, is_async=True)
            )
        return future
    ...
    def fit(self):
        ...
        while True:
            ...
            with marked_timer("step", timing_raw):
                ...
                if self.use_dataset_teacher_router and "teacher_name" in batch.non_tensor_batch:
                    teacher_names = [str(name) for name in batch.non_tensor_batch["teacher_name"].tolist()]
                    distill_weights = batch.non_tensor_batch.get("distill_weight", None)
                    teacher_counts = {}
                    teacher_response_lens = {}
                    teacher_weight_sums = {}
                    for idx, teacher_name in enumerate(teacher_names):
                        teacher_counts[teacher_name] = teacher_counts.get(teacher_name, 0) + 1
                        teacher_response_lens.setdefault(teacher_name, []).append(response_lens[idx])
                        if distill_weights is not None:
                            teacher_weight_sums.setdefault(teacher_name, []).append(float(distill_weights[idx]))
                    for teacher_name, count in teacher_counts.items():
                        metrics[f"teacher_samples/{teacher_name}"] = count
                        teacher_lens = teacher_response_lens[teacher_name]
                        metrics[f"response_seq_len/{teacher_name}_average"] = sum(teacher_lens) / len(teacher_lens)
                        metrics[f"response_seq_len/{teacher_name}_max"] = max(teacher_lens)
                        metrics[f"response_seq_len/{teacher_name}_min"] = min(teacher_lens)
                        if teacher_name in teacher_weight_sums:
                            weights = teacher_weight_sums[teacher_name]
                            metrics[f"distill_weight/{teacher_name}_average"] = sum(weights) / len(weights)
                    batch.meta_info["teacher_name_to_id"] = self.teacher_name_to_id
                    batch.meta_info["teacher_id_to_name"] = self.teacher_id_to_name
