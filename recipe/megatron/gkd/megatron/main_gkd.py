...
from recipe.gkd.megatron.dataset_teacher_router import collect_train_files, load_dataset_teacher_router
from recipe.gkd.megatron.ray_trainer import OnPolicyDistillTrainer
from recipe.gkd.megatron.routed_rl_dataset import RoutedRLHFDataset
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        if config.data.get("dataset_teacher_router", None):
            router_cfg = load_dataset_teacher_router(config.data.dataset_teacher_router)
            with open_dict(config):
                config.data.train_files = collect_train_files(router_cfg)
            dataset_cls = RoutedRLHFDataset

        # Create training and validation datasets.
        # train_dataset = RLHFDataset(config.data.train_files, tokenizer, config.data, None)
        train_dataset = dataset_cls(config.data.train_files, tokenizer, config.data, None)
