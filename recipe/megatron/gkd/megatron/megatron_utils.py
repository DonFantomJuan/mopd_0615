    ...
    for cur_pp_rank, scan_vpp_idx, idx, name, tensor_spec in layer_list_meta:
        ...
        def gather(tensor, gather_list, group, group_dst, group_rank):
            if group_rank == group_dst:
                torch.distributed.gather(tensor, gather_list, group=group, dst=group_dst)
            else:
                torch.distributed.gather(tensor, None, group=group, dst=group_dst)
        ...
        if tp_utils.is_tensor_parallel_param(cur_tensor):
            ...
            if tp_size <= 1:
                ...
                torch.distributed.all_gather(infer_params, cur_tensor, group=tp_group)
